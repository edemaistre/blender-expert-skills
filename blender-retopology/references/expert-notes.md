...
- The map is drawn as Grease Pencil layers named, in his file, `edge flow`, `creases primary`, `creases secondary`, `articulation`, `expression bounds`, `landmarks` (layer panel visible in every frame from 00:00:45). What the frames show:
  ...
  - **Primary creases** (cyan, 00:09:23): the lid creases around both eyes, the nose wing, the lip outline. Modelled into topology so they stay crisp [00:09:01].
...
- Rain's head coloured by face sets per patch (00:05:17): purple eye masks, pink nose and mouth, orange nasolabial area, green cheeks, blue skull: patches are the unit of design. Seams (red) mark patch borders and articulation loops, sharps (cyan) mark crease edges, as reminders (frames 00:00:45, 00:04:32, 00:15:23).
...
- Crease = centre loop + two framing loops; "the closer they hug this edge the sharper the crease" [#1 00:34:37] [02:13:08]. A brow crease only needs cross-hatching loops [#1 01:15:06].
...
- Rigor scales with deformation: exact counts at mouth and eyes, ears only need non-spiralling loops, hidden ear parts may be messy [#2 01:33:38] [01:38:30].
...
- A bulge needs at least one loop or the subdivided shell collapses [#3 01:05:50]; hard edges between volumes by bevel (two close edges crease) [#3 01:09:41]; lay out everything before bevelling [#3 01:18:27].
...
- Arm from an 8-vertex circle centred with the cursor snapped to Volume, "go as low as possible" [#4 00:06:16]; 3 loops per joint minimum [#4 00:08:01]; leg cylinder 8 then 12 [#5 01:21:00] [01:22:09]; knee inset twice so the sharp edge sits between two loops [#5 01:26:56].
...
- Pose-independent folds modelled into the cage so animators see them unsubdivided [Finale 00:27:25]. Triangles in pairs so they merge into quads [Finale 00:28:32].
...
- Equal spans top and bottom of lids and lips, per quadrant (example 10/10, 3 per quadrant), non-negotiable for rigging; exact counts are circumstantial [face 00:03:42] [00:04:16]. Spiralling eye or lip loops mean rebuild; the face centre line is one loop with no poles [00:04:49].
...
- Hard edges vs UV splits were never discussed (he shades all smooth); the baking rule "every hard edge is a UV seam" comes from On Mars 3D [C_RqdNbYOjE 00:08:04] and Polycount (see blender-uv-baking).
...
- CG Boost: snapping handles hand-moved vertices, the Shrinkwrap catches operator-made geometry (loop cuts) [X2GNyEUvpD4 00:08:04]; face snapping projects from the view [00:07:02]; important loops first, unconnected, low resolution [00:09:36]; finalize: apply Shrinkwrap and Mirror, check centre merges [00:13:01].
...
## 9. The sheep experiment: what an agent reaches (/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-retopology/test_sheep.py)
