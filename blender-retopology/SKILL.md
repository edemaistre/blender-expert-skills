...
**REQUIRED BACKGROUND:** blender-expert (execution channel, review loop, 5.2 API changes). Toolkit: `scripts/bx_retopo.py` (`import sys; sys.path.append("<skills>/blender-retopology/scripts"); import bx_retopo as R`), all coordinates in the sculpt's local space.
...
## What an agent reaches (measured on the sheep; `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-retopology/sheep_results.json` and `compare/metrics.json`)
...
| Raw `quadriflow_remesh` on a small dense mesh | CANCELLED, "needs to be manifold" | `R.quadriflow` (scale trick, voxel fallback) |
...
## References
- `references/procedures.md`: tested code, P0 to P14 (inspect, face sets P1b, stack, tubes, geodesic limbs P3b, freeze/relax, annotations, QuadriFlow, map-line loops and rings, sockets, volume, checks, game, expression test, handoff, AI-mesh finishing). Load before writing any code.
- `references/expert-notes.md`: principles by expert with timestamps, face and body loop maps, clothing, game rules, the sheep experiment in full. Load when planning a face, body, clothing or game mesh.
- `references/critique.md`: the rubric to judge your own retopo. Load at every review.
- `references/sources.md`: all 24 source videos with best timestamps.
