...
2. **Seams.** Hard surface: `seams_from_sharp(obj, 30, mark_sharp=True)`. Organic: build part labels (head, ears, limbs, torso front and back, soles, inner mouth and eyelids per the Kaspar and Morren seam maps in `references/expert-notes.md`), then `seams_from_groups(obj, labels)`. Specific cuts: `seam_path(obj, v_a, v_b, view_dirs)`. Then `cut_to_disks(obj, view_dirs=[camera])`. GATE: `must_cut` empty; a checker render shows no seams across hero areas that the brief cannot afford.
...
7. **Look.** `uv_layout_image`, `checker_review`, `distortion_review(metric="angle")`. Judge with `references/critique.md`.
...
| Target node active but not selected | bake returns CANCELLED, no error | select and activate; assert FINISHED |
...
| One global extrusion on thin, close parts (ears on a head) | inverted patches, neighbours baked in | adaptive bake (default), explode, or per-pair bakes |
...
- `references/procedures.md`: tested pipelines (hard-surface, character, film/UDIM, Multires, bake-fix loop) and raw bpy snippets executed by the tests. Load before writing code.
...
- `references/sources.md`: videos, credentials, URLs, best timestamps, web references.
