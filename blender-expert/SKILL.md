...
Check `bpy.app.version` first; this skill set targets 5.2 LTS. Many tutorials the experts recorded use 2.8x to 4.2 UI: read `references/blender-5.2-deltas.md` before trusting any remembered operator, brush name or API.
...
| Brush strokes (`sculpt.brush_stroke`, `paint.image_paint`, `paint.weight_paint`) | no (poll fails) | yes, via `scripts/bx_gui.py` |
| `paint.mask_flood_fill`, `sculpt.mesh_filter` | CRASHES Blender | yes, via `bx_gui.run()` |
| Programmatic sculpting on mesh data | yes, `blender-sculpting/scripts/bx_sculpt.py` | yes |
...
|---|---|---|
| Character artist, stylized or realistic | blender-sculpting, blender-retopology, blender-uv-baking, blender-texturing-shading, blender-hair | sculpt, animation-ready low poly, UVs, baked maps, materials |
| AI-mesh finisher (Scenario 3D output to production) | audit the generated mesh, blender-retopology (or cleanup + remesh), blender-uv-baking (bake FROM the generated high), blender-texturing-shading; generation itself: scenario-3d | clean, UV'd, baked asset at a stated budget |
| Game asset artist | blender-hard-surface or blender-sculpting, blender-retopology (game budget), blender-uv-baking, blender-texturing-shading | low poly + PBR set, exported glTF/FBX |
| Rigger / character TD | blender-rigging | rig with controls, weights, deformation tests |
| Animator | blender-previs-storyboard (shot planning), blender-animation | blocked, splined, polished shot + playblast |
| Story / previs artist | blender-previs-storyboard, blender-grease-pencil | shots, cameras, animatic |
| Look-dev and lighting artist | blender-texturing-shading, blender-lighting-rendering | lit, color-managed, composited renders |
| Technical artist / procedural | blender-geometry-nodes | node-group assets, procedural systems |
| 2D / 2.5D animator | blender-grease-pencil | GP animation |
...
## References
- `references/bpy-reliability.md`: checklist and tested procedures for reliable bpy (operators vs data API, stale references, evaluated vs original data, idempotent headless jobs, numpy bulk access, node trees, persistence). Load before writing any non-trivial script.
