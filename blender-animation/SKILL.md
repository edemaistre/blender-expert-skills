...
**REQUIRED BACKGROUND:** blender-expert (execution channel, review loop, 5.2 API changes). Shot planning, cameras and animatics belong to blender-previs-storyboard; building the rig to blender-rigging.

Toolkit: `scripts/bx_anim.py` (keying, blocking-to-spline, curve checks, measurement, staging, playblast light, knee-pop, flip and balance checks, onion-skin and curve images, one-object `bouncing_ball`; API in procedures.md, tested on 5.2.1) and `bx_review.playblast`.
...
- **Odd frames, even gaps; breakdowns shape arcs and steer the eye.** Alex Nagy: key poses on odd frames with even spacing so a true middle frame exists. The breakdown sets the arc first, the timing second; favouring it late leaves more travel for the last frames, and the fastest part draws the eye.
...
- **Spacing is felt, arcs are checked, the shot must read.** Raymond: the audience should feel spacing tricks, not see them. Pablo Fournier: arcs are the most important check (root, hands, feet, nose), in camera space. Hjalti (in blender-previs-storyboard): after each cut the eye must find one thing, so a subject lost in an empty frame fails however good the curves are.
...
## Workflow
1. **Plan.** Beat list with frames: every acting beat in order (Hjalti counts about 8 per shot), sub-beats notice, react, realise, respond (Rik), the leading part per beat. With audio, `audio_envelope` + `accent_frames`; hit a pose just before its audio peak (Alex). Start mid-action (Rik). GATE: beat table written.
...
6. **Re-inject energy** (Leo: splining softens): timing accents, overshoots, desynchronised parts, `offset_keys` 1 to 2 frames per chain link (Dillon), head lag 2 to 3 frames in cycles (Joey), contacts that slide rather than glue (Rik). Fingers: little overlap, no bounce. GATE: arcs clean in camera pixels (`to_camera(..., res=(w, h))`, `motion_report(dims=2)`).
7. **Face and lip sync, stepped** (P6, P7): jaw rhythm, corners (never dead centre), shapes from a pose library on the lower face minus the jaw, accents and asymmetry in small doses, tongue last, then spline the face. Eyes dart in 1 to 2 frames and lead the head; brows 3 to 10 frames (Rik). GATE: `transitions` meets those counts; accents clearly fewer than syllables.
...
| Arcs checked in normalised camera coords | false sharp turns at apexes (aspect squeezes x) | `to_camera(..., res=(w, h))` |
...
## References
- `references/expert-notes.md`: principles, numbers and disagreements by expert, with timestamps; load when choosing between approaches or writing notes.
- `references/procedures.md`: full tested code for every procedure (setup, blocking, spline, walk, lip sync, facial timing, contact pin, slots, layers, pose library, playblast, measurement, ball, staging, knee/flip/balance); load before writing code.
- `references/critique.md`: the review rubric per stage, staging thresholds and their calibration, how these animators phrase notes; load at every gate.
- `references/sources.md`: the 14 source videos, what each is best for, best timestamps.
