...
**REQUIRED BACKGROUND:** blender-expert (execution channel, review loop, 5.2 API changes). Related: blender-texturing-shading (materials, skin SSS), blender-previs-storyboard (shot planning, cameras).

Toolkit: `scripts/bx_light.py` (tested on 5.2.1): rigs placed relative to subject and camera, EEVEE/Cycles presets, color setup and audit, 5.x compositor helpers, render analysis. Import: `sys.path.append("<this skill>/scripts"); import bx_light as BL`.
...
8. **Comp.** Cycles: `mix = BL.lightgroup_mix(gains, exr=r["exr"])` re-grades key/fill/rim/window from the EXR in about 0.2 s (`BL.set_gain`, RGB gains recolour). Then `BL.comp_finish(bloom=0.25, vignette=0.3, contrast=0.05, source=mix["socket"])`; masks with `BL.matte(tree, names)`; pixel sizes through `BL.relative_px`; passes, back to beauty and view layers are in procedures P13. GATE: re-run the verdict on the finished frame; the vignette ratio should fall.
...
| Brand colour error (of 255): Khronos 3.8, AgX 27 to 42, Standard 33 | white furnace | measured |
...
```
Visual: open the render, the `value_study` panel (desaturated | value groups | squint) and, for sequences, the contact sheet. Judge them with `references/critique.md`: first read, value structure, form, motivation, color, composition, artifacts, finish.
...
| EEVEE interior without probe, or a probe box drawn to the walls | Shadows lifted blue/grey everywhere, muddy | Outer samples at the inner walls (`probe_report`), bake, sealed-room test |
...
## References
- `references/procedures.md`: tested code for every stage (P1 units to P17 exposure and colour), with measured results and test paths. Load before writing code.
...
- `references/expert-notes.md`: principles per expert with timestamps, plus the disagreements table. Load for judgment calls and attribution.
- `references/sources.md`: the 15 videos, credentials, best timestamps, and which to consult for which question.
