...
Every procedure below ran headless with `blender --background --factory-startup --python-exit-code 1 --python <test>`. Tests live in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-lighting-rendering/` (fixture: `_common.py`, a stylized character in a 4 x 3.5 m workshop with 0.2 m walls, a window and a hanging lamp). The module is `skills/blender-lighting-rendering/scripts/bx_light.py`; import it with:

```python
import sys; sys.path.append("<skills>/blender-lighting-rendering/scripts"); import bx_light as BL
```
...
A light's *level* = the linear value a white Lambertian card facing the light at the target reaches. Measured constants (EEVEE and Cycles agree to 0.1 % with custom distance on):
...
- AgX looks: None, Punchy, Greyscale, Very High / High / Medium High / Base / Medium Low / Low / Very Low Contrast (all prefixed `'AgX - '`). ACES 1.3/2.0 have no looks.
...
- **View transform by job** (Jacob [00:45:24]; Gleb [6hPZ0ckL5I0 00:21:42]): AgX for scenes; `'Khronos PBR Neutral'` when a product or brand colour must display as authored; ACES 2.0 or AgX High Contrast for portraits (Gleb: Khronos oversaturates skin). Measured in a white furnace (Principled plane, base colour = the brand sRGB, test_12), max channel error in 8-bit steps:

| Brand colour (sRGB) | Standard | AgX | Khronos PBR Neutral |
|---|---|---|---|
...
| orange (250, 120, 0), above the knee | 54.3 | 77.0 | 37.3 (compressed) |
   ...
   Khronos removes the dielectric specular lift that makes Standard look washed out; colours whose brightest channel exceeds about 0.76 linear (Khronos spec knee) are compressed, and a zero channel is lifted by its toe.
...
BL.clay(True); ...; BL.clay(False)                         # view_layer.material_override grey
...
```
   Raw sizing: `N = round(2 * inner_half / cell) + 1`, `half = inner_half * (N + 1) / (N - 1)` per axis, `probe.scale = half`. Keep walls thick and `escape_bias` small; re-bake after material changes; seal the windows to test.
2. **Per-light jitter** on a soft key: head 5th-percentile luma 0.0060 without, 0.0044 with (contact shadows deepen). `scene.eevee.use_shadow_jitter_viewport` is viewport only.
3. **Fast GI steps** 8 vs 16: frame mean 0.0398 vs 0.0400 on 5.2.1 (Andrew's "more steps, brighter" is marginal after the 5.2 overhaul; keep 16, it costs little).
4. **HDRI sun threshold applies after strength** (Andrew [00:51:15]): synthetic HDRI with a 2000 sun: at strength 1 threshold 10 vs 0 changes the image (2.66 to 4.19 mean); at strength 0.004 (sun 8 < 10) the two are identical. The extracted sun's cast shadow did not show in this top-down setup (not verified).
5. Factory facts: `use_raytracing` False; `gi_cubemap_resolution` '512'; Cycles `sample_clamp_indirect` 10, 12 bounces, caustics on.
...
Verified: `test_08_s7_hero.py` (outputs in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/lighting_skill/skill_out/test_08_s7_hero/`).
...
lin = BL.rebalance(r["exr"], {"key": 1.6, "window": 0.5, "rim": 2.0})         # numpy, no render: iterate with analyse()
...
BL.set_gain(mix, "key", (1.2, 1.1, 1.0))                                      # RGB gain = recolour a light (Pau)
...
## P17. Exposure, product colour, light-linking location, face key size
...
BL.color_setup(view="Khronos PBR Neutral")                               # product or brand colour shots
