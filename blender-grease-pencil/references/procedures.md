...
Every block below ran on Blender 5.2.1 LTS (`blender --background --factory-startup`) unless marked GUI only. Raw-bpy blocks P1 to P8 are copied verbatim from `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-grease-pencil/test_procedures.py` (between `# >>> Pn` and `# <<< Pn`), which asserts their results. bx_gp recipes are exercised by `test_bx_gp_2d.py`, `test_bx_gp_25d.py` and, in a live session, `test_gui_draw.py`. Outputs of the last runs: `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/grease_pencil_skill/`.

Conventions: drawing plane XZ, camera at -Y looking +Y, metres, `radius` = half line width, material colors scene-linear (bx_gp accepts sRGB hex).
...
for p, co in zip(s.points, ring(1.0)):
    p.position = co
    p.radius = 0.03        # HALF the line width, metres
    p.opacity = 1.0
s.cyclic = True
...
cd.type = 'ORTHO'
cd.ortho_scale = 4.0                                  # frame width in metres (larger image side)
...
la.radius = 0.01                                      # = line WIDTH in metres (points get radius/2)
...
Verified on 5.2.1: `test_procedures.py` P7 (one layer `Layer`, `fill_id` 1, radius 0.03 carried over); render of the same graph in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/grease_pencil/probe4c_render.png` (distiller).
```python
...
```python
import math, sys
sys.path.append("<skill>/scripts")                            # blender-grease-pencil/scripts
import bx_gp as G
