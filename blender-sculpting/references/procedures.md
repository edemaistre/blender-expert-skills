...
Every block below ran on Blender 5.2.1 LTS on 2026-09-24. Test scripts live in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-sculpting/`:
...
```python
import sys
sys.path += ["<skills>/blender-sculpting/scripts", "<skills>/blender-expert/scripts"]
...
```
`lid()`: upper margin 21 degrees above the eye centre, lower 26 below, rolled 8 degrees (outer corner up, Thelen 00:46:10), thickness 0.012 H, back clip at the eye centre plane. Measured failures: margin 14 degrees read sleepy; a socket 1.2 x the eyeball left a ring groove that read as eye bags; a back clip behind the centre let the two margin planes meet behind the eye (at y = +0.061 r) and seal a pocket (3 components).
...
target, cam_loc, ppu = S.framing(objs, (0, -1, 0), lens=90, margin=1.08)                 # ppu * res = px per metre
```
...
```
The attribute survives voxel remesh with preserve attributes on (distiller, `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/sculpt_studio_anatomy/probe5.py`).
...
```
Multires belongs on a settled clean base like this one, never as a blockout tool (Henning Cmi0KoFtc-4 00:16:04). Production retopology: blender-retopology.
...
| Inflate, Pinch | along own normal; toward centre | `dab("inflate" / "pinch")` | cheeks; tighten edges |
