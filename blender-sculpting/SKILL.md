...
- **Stylize by consistent exaggeration** (Yan: pick it in minute one, make neighbours follow; a long face needs a smaller skull, not a stretch).
...
| Purpose | still (vertex paint, decimate) vs game or film (retopology) | still; blender-retopology if animated |
...
| Eyes (stylized) | radius 0.064 H, lids 0.012 H thick, upper margin 21 degrees above centre | example; 14 degrees read sleepy |
...
| Landmarks | eye line 0.5 H; nose bottom halfway brow to chin; lip parting 1/3 nose to chin; ear nose base to brow; width H/1.5; mouth corners under eye centres; canthal tilt about 8 degrees; neck about 15 degrees forward | Abbitt, Naydenov, Thelen |
...
- **Visual (the gate):** `S.long_lens_render` or `final_render` at 85 to 95 mm from front, profile, three-quarter and below, plus `bx_review.review(..., modes=("silhouette", "matcap"))` for the ortho silhouette. Order: silhouette, big planes (cavity off, or `mode="flat"`), below and above, lines, character read at thumbnail size. Rubric: `references/critique.md`.
...
| Lids reaching behind the eye equator | sealed pocket, 3 components | clip lids at the eye centre plane |
...
| Symmetrizing asymmetric parts | swept hair mirrored, centre seam | `to_object(..., symmetric=False)` |
...
## References
- `references/procedures.md`: tested code for every stage (Clay recipe, planes, form check, hair, lids, expression, final render, crash matrix, brush table, GUI recipe); load before writing any sculpt script.
- `references/expert-notes.md`: principles by expert with timestamps; load when designing a character or choosing methods.
- `references/critique.md`: stage rubric with blockers; load at every gate.
- `references/sources.md`: the 27 source videos with best timestamps.
- Example: `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-sculpting/stylized_head_blockout.py` (13 s, renders in `stylized_head/stage_N/`); comparison: `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-sculpting/compare/compare_sheet.png`.
