...
checks for Blender 5.2.1 (skill: blender-texturing-shading). Every function below is
exercised by /Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/test_*.py.

  import sys; sys.path.append("<skills>/blender-texturing-shading/scripts"); import bx_materials as M
...
Live GUI session (real brushes through blender-expert/scripts/bx_gui.py):
...
_EXPERT = os.path.normpath(os.path.join(_HERE, "..", "..", "blender-expert", "scripts"))
...
def edge_wear_group(name="BX Edge Wear"):
    ...
    EEVEE or a game engine bake these outputs (blender-uv-baking) or use
    curvature_attribute()."""
...
def add_wear(mat, edge_color=None, edge_roughness=None, edge_metallic=None,
    ...
    to textures first (blender-uv-baking), then threshold the baked mask."""
    mat = get_material(mat)
    nt = mat.node_tree
    p = principled(mat)
