...
The judgment behind SKILL.md, by expert, with video id and timestamp (`[hh:mm:ss]`). Video ids resolve to `https://www.youtube.com/watch?v=<id>`; details in `sources.md`. [added] marks the skill author's own additions; "measured" marks facts established by the tests in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/`.
...
- Flat base colours per material first, then black/white masks mixed in nodes, "very non-destructive, like Substance" (0vGNOtX9iPk [00:05:14], [02:04:38]). Masks are later reused for roughness, SSS and bump (0vGNOtX9iPk [02:05:50]).
...
- Vertex colours vs texture paint: vertex colours for broad soft masks on a dense retopo (fast, blur works, no UVs) (0vGNOtX9iPk [01:19:25]; D7kuKx7BV1A [00:21:38]); texture paint when vertex resolution shows (hands) or for height and streaks (kN5aq1qXUCE [00:58:32]; uTjfRGGV6ys [00:55:37]). Deciding condition: mask frequency against vertex density.
- Colour attributes are per face corner: face masking gives hard borders (nails), vertex masking soft ones (0vGNOtX9iPk [02:06:31]).
- Name colour attributes `col_*`: attribute pickers mix them up with UV maps (kN5aq1qXUCE [01:34:39]).
- Keep the whole character at a consistent level of progress, not the skin finished first (kN5aq1qXUCE [01:08:24]).
- Organise as you go: frames, labels, colour codes, node groups, named reroutes; a shared mask becomes a node group duplicated next to each consumer instead of long crossing links (kN5aq1qXUCE [00:42:16]; EDEMbeZfVcc [00:07:09], [00:08:17]).
...
- Neutral mid-grey non-gradient background; an HDRI like the forest one tints everything green (0vGNOtX9iPk [00:13:20], [01:58:52]).
- Isolate channels by unplugging or muting links: base colour to black to read bump and reflections, normal off to read roughness (kN5aq1qXUCE [01:26:09]; D7kuKx7BV1A [01:19:36]). `M.preview(rigs=("roughness",))` automates this.
...
- SSS colour inherited the darkening masks of the base colour network and darkened the upper lip; he duplicated the network into its own "SSS" frame (D7kuKx7BV1A [00:24:31]). 5.2 has no Subsurface Color: the SSS albedo is Base Color; redden via Subsurface Radius or a separate Subsurface Scattering node [added].
- Fact-check the SSS colour against subdermal references: redder than the surface (D7kuKx7BV1A [00:10:21]).
...
- Wet mouth without touching specular: keep 0.5, tongue roughness about 0.4 with strong waxy SSS, teeth roughness 0.4, slight SSS, cream colour (D7kuKx7BV1A [00:50:05], [00:50:52]).
- Paint region masks as black/white plus Map Range, not absolute painted values that get colour managed (D7kuKx7BV1A [01:21:56]).
...
- Stubble and hairline: keep flat, gradient to skin colour bottom to top, a little detail for directional shine, a touch of SSS to blend into flesh (8GD1C9qLi-4 [02:02:25], [02:07:46], [02:11:20]).

### Eyes (Snow #5)
- Procedural rather than painted; Rain's painted iris looked worse and was less adjustable (kAdgrBS4eDA [01:04:50]).
- One object per eye with its origin at the eyeball centre, mirror modifier removed, stray keys cleared; object-space gradients break if the eye is stretched in edit mode (kAdgrBS4eDA [01:12:07], [01:17:07], [01:25:07]).
- Spherical gradient in object space, two Color Ramps for iris and pupil, a Linear Light mix with noise to break the circles without moving them, darker limbal rim via Multiply (kAdgrBS4eDA [01:10:25], [01:19:30], [01:21:40]). Implementation note [added]: a gradient centred on the eyeball centre is constant over the eyeball surface, so `stylized_eye` measures distance from the front pole instead.
...
- 5.2 translation, measured: EEVEE needs Material Output Thickness 0 on the cornea shell or it magnifies the iris across the eye; `visible_shadow = False` is honoured by EEVEE and Cycles.
...
- Use a colour map only as pattern: saturation 0, value lowered, multiplied into the flat colour (kN5aq1qXUCE [01:17:00]).
...
- One painted 0.5-based colour-variation map split by Color Ramps into a bright (Add wash) and a dark (Multiply) mask; height times fabric structure through a high-contrast ramp gives acid wash on raised areas only (rKVeLjxZzTc [02:04:12]; kAdgrBS4eDA [00:24:56]).
...
- Bake recipe: Emit via a Node Wrangler preview, margin 8 px, 30+ samples (1 only for flat colour, "one sample is not a good idea for normal maps"), disable the Mask modifier's render visibility, validate by rebuilding the material from the maps (EDEMbeZfVcc [00:31:58], [01:21:12], [01:36:11], [00:45:28]). Details in blender-uv-baking.
...
- Global clay and wire switch: a node group in every material driven by one custom property (clay: diffuse 0.6 grey, SSS 0, roughness 0.6) (EDEMbeZfVcc [00:15:26], [00:17:04]).
- Camera-ray world: camera sees flat grey, the HDRI lights the character; "a really neat trick that I'm basically always using" (8GD1C9qLi-4 [00:35:14], [00:35:50]).
...
- Viewport display colours: pick each material's diffuse colour into `diffuse_color` for animators (kAdgrBS4eDA [01:05:09]).
...
- Texture paint: strokes issued before a redraw are occluded by the previous draw's depth buffer (162 texels in one run, full coverage in another; about 14,000 after a redraw every run); scripted strokes place one dab per point; factory paint colour black.
...
- Every map but base colour is data, Non-Color: "it's not about what looks better or what you can see, it's what's actually correct" [00:21:29], [00:22:03].
...
### PBR base
- A texture is a per-pixel slider: a flat 0.2 grey Non-Color image into Roughness equals Roughness 0.2 [00:02:50].
...
- Download: 8K for a large wall, JPEG is fine for colour and data, displacement 16-bit TIFF; skip ORM (for real-time) [00:15:32], [00:16:07].
- AO: Mix Color Multiply, A = base colour, B = AO, factor 1 ("crank it all the way up almost always"): the flash-lit capture removed cavity shadows; optional under true displacement [00:22:36]-[00:26:23].
...
- Normal maps survive 8-bit; greyscale displacement needs 16-bit; bump derived from a greyscale map is worse than a normal map (slope from neighbouring pixels) [00:37:36].
...
### Masks and layers
- Colour tweaks non-destructively: HSV after the AO multiply, hue shifts of about 0.04 only; frames show saturation 0.9, value 1.2 to 1.4 [00:47:10]-[00:50:22].
...
- Efflorescence: the same group with a flipped ramp in Screen (Add exceeds 1) [01:38:07]-[01:40:48]. Spalling: Subtract on the height before the Displacement node (factor about 0.14) and brighten the same area's colour because fresh breaks have no grime [01:44:33]-[01:50:57].
...
- Object Info Random into a Color Ramp gives each duplicate its own colour from one material [5B244CYX1Tw 00:45:06]. Pointiness is a free cavity/edge mask, Cycles only, density dependent [00:41:19] (measured: EEVEE constant).
- A mostly mid-grey factor blends weakly: add contrast before using it as a mask [5B244CYX1Tw 01:00:03] (measured: Noise Fac clusters in 0.3 to 0.7).
- Never plug height into Normal: Bump node [5B244CYX1Tw 00:54:20].
- Painted bump: a 0.5-grey image through a Bump node; black carves, white raises; view in Material Preview [4d4N8d4ki2Y 00:44:43]-[00:47:24].
- Islands as large as possible, no overlaps, inside 0..1; fill a new image with the dominant colour; 4K for a hero stylized prop [6jT4K0JpGNs 00:04:36]-[00:08:39]. Stroke spacing and jitter for dots, Line stroke for straight lines [6jT4K0JpGNs 00:18:59].
...
- NoPoly (nQaUsgf9ZUg, self-described non-expert): UV coordinates for images, Object for procedurals [00:07:21]; the Mix factor only reads black and white, colours go in A/B [00:18:12]; a stretched 3D procedural (wood grain, Mapping scale (3, 3, 0.1), distortion 2, detail 5) only reads right on two axes [00:10:14], [00:11:20].
- YanSculpts (kbyK_N65YEk): course trailer, no technique; only a layer order for painted skin (base, variation, imperfections, freckles; separate grey maps for bump, specular and SSS) [00:02:24].
...
| Where masks live | Kaspar, Price: vertex colours; Abbitt, Ryan, Surfaced: images | mask frequency vs vertex density; tiling UVs and changing layouts favour vertex colours; export and fine detail favour images |
