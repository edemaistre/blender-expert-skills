...
Every `python` block below was executed on Blender 5.2.1 LTS by `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/test_procedures_md.py` (headless, `--factory-startup`, fresh file per block). Blocks marked `python gui` need a live window and were run by `test_gui_texture_paint.py` instead. Larger behaviours were verified by the render tests named under each procedure; their contact sheets are in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/out/`.
...
Verified on 5.2.1. Test: `test_pbr.py` (ambientCG, Poly Haven and Substance naming, colour spaces, glTF export).
...
mat = M.pbr_from_textures("Bricks", maps, uv_map="UVMap")   # sRGB on colour, Non-Color on data, sockets by name
obj.data.materials.append(mat)
print(M.audit_material(mat, obj))              # [] when colour spaces, UV maps, bump and emission are sane
```

What the builder does, so you can do it by hand when needed:
- Role from the END of the file name (`metal_plate_diff_1k` is base colour, not metallic). `_nor_dx` / `NormalDX` sets the Normal Map node `convention = 'DIRECTX'` (5.1+).
...
- Height: bump only when there is no normal map (`height_as='auto'`); Bump Distance = `height_depth` in metres (what 0..1 of the map spans). A normal map plus bump is CHAINED (Normal Map output into the Bump Normal input), never added. `height_as='displacement'` wires true displacement and DROPS the normal map (Price: they encode the same relief; `audit_material` flags materials that use both), sets the height image to Cubic interpolation and `displacement_method = 'BOTH'`.
- AO: multiplied into base colour at factor 1 by default (Price: scanned sets are flash-lit, AO restores the cavity shadow; optional under true displacement). For games use `ao_mode='gltf'`: a `glTF Material Output` group so the exporter writes a separate `occlusionTexture` (verified in the exported JSON); the default multiply still exported `baseColorTexture` (verified). `ao_mode='none'` to skip.
- Emission map: Emission Strength set to 1 (5.x default is 0).

Colour-space fix for an existing material, by role of the socket it feeds:
...
p.inputs["Subsurface Scale"].default_value = 0.02       # metres; radius = Scale x Subsurface Radius
p.inputs["Coat Weight"].default_value = 0.0             # old 'Clearcoat'
p.inputs["Emission Color"].default_value = (1, 0.5, 0.1, 1)
p.inputs["Emission Strength"].default_value = 2.0       # default 0: colour alone does nothing
...
## P4. Wear, dirt and colour variation on any material

Verified on 5.2.1. Test: `test_wear.py` (mask sheet Cycles vs EEVEE, preview, value report) and `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/probes/probe_stochastic.py`.
...
- Bevel and AO are sampled per render sample: a threshold on them averages to fractional values. Metallic driven through `GREATER_THAN` was 0% fractional at 1 spp and 17 to 18% at 16 and 128 spp. Bake Wear and Dirt to textures (blender-uv-baking, Emit bake) before thresholding for finals, EEVEE and games.
...
Verified on 5.2.1. Tests: `test_wear.py` (EEVEE and Cycles identical, mean 0.3402 vs 0.3404), `test_uvpaint.py` (rasterised into a texture).
...
# Blender's own cavity mask: works headless in OBJECT mode on the ACTIVE colour attribute
...
Both depend on vertex density (like Pointiness). On a low poly, compute them on the high and bake, or rasterise per texel with `M.vertex_values_to_texels` (P11).
...
Faces facing the decal get planar UVs; every other loop is parked outside 0..1 and the image node uses Extension CLIP, so it reads alpha 0 there. For games, a separate decal mesh floating a millimetre above the surface is the usual alternative [added].
...
- Base colour: start darker, brighten through `col_brighter`, darken through `col_darker`, lips mask removed from both, lips colour on top.
...
- Roughness: base 0.68 to 0.72; pores from two Voronoi Smooth F1 textures (scale 100 and 200 on his UVs) mixed by a detail-0 noise, general noise mixed at about 0.2, pore centres glossier, cracks rougher; `col_rough` remapped to 0.72 to 1 (chin, brows); lips glossy (0.45 here; his tongue and teeth 0.4).
- Specular IOR Level stays 0.5: wetness is roughness.

Backlit SSS check: `M.preview([head, ...], out, rigs=("backlit",))` puts a strong area light straight behind plus one from below (Kaspar's rig). Ears, nostrils, lids, lips and fingers should glow red; forehead, jaw and palm centre stay opaque.

## P8. Procedural eye and cornea

Verified on 5.2.1. Tests: `test_skin.py` (eyes on the head, cornea shadow per engine), `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-texturing-shading/probes/probe_cornea.py` (EEVEE thickness variants).
...
r = 0.012
bpy.ops.mesh.primitive_uv_sphere_add(segments=48, ring_count=24, radius=r)   # origin = eyeball centre
...
- Factory 5.2: brush `Paint Hard`, colour AND unified colour black, `use_unified_color` True, seam bleed 2 px, mode `MATERIAL`.
...
Image value semantics (measured by writing 0.5 and reading the PNG): byte images store values as encoded (0.5 becomes 128 whether sRGB or Non-Color); float images hold linear and a `Linear Rec.709` float saved to PNG is converted to sRGB (0.5 becomes 0.735); a float Non-Color image saved to 16-bit PNG keeps 0.5. Write alpha 1 into maps without real alpha: an RGB PNG save multiplies colour by alpha (0.5 with alpha 0.5 saved as 64).
...
Use the result as a Non-Color mask and bump; build colour, grunge and wear procedurally on top.
...
obj.data.materials.append(mat)
print(M.audit_material(mat, obj))                          # flags the grey metallic
...
`preview` rigs: `studio` (key, fill, rim on neutral 0.18 grey, no gradient), `backlit` (Kaspar's SSS test), `roughness` (studio with Base Color forced black and SSS off: reads roughness and bump alone). Engines side by side catch EEVEE-only or Cycles-only nodes.
...
The camera sees flat grey, the object is lit by the HDRI.
...
mix.inputs[7].default_value = (0.6, 0.6, 0.6, 1)       # B = clay grey (RGBA mode: A=inputs[6], B=inputs[7])
...
## P18. Environment grime from a numpy colour-attribute mask plus Color Burn (Price)

Verified on 5.2.1. Test: `test_price_masks.py` (plain soft mask vs burned mask on a brick wall, EEVEE and Cycles; the burned mold concentrates in the grout). Price paints the mask in vertex colours (resolution = vertex count, he subdivides a wall with 20 cuts) because image painting on tiling UVs repeats every stroke. Headless, the mask comes from a rule instead of strokes.
...
ramp = nt.nodes.new("ShaderNodeValToRGB")            # contrast: scanned height sits in mid greys
...
mold.inputs[6].default_value = (0.3, 0.1, 0.07, 1)   # A: the existing base-colour chain in a real material
...
tile = 2.5                                            # metres per texture tile (vendor data)
