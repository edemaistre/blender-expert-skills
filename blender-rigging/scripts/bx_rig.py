...
headless (`blender --background --factory-startup`) and in a live session.
  ...
  import sys
  sys.path.append("<skills>/blender-rigging/scripts"); import bx_rig as R
...
def mannequin(name="Mannequin", height=1.8, pose="T", head_scale=1.0, eyes=True, shirt=True,
    ...
    sys.path.append(os.path.join(SKILLS, "blender-sculpting", "scripts"))
    import bx_sculpt
    H = height
...
def deformation_report(rig, meshes, poses=None, self_intersections=True):
    ...
    if self_intersections:
        sys.path.append(os.path.join(SKILLS, "blender-expert", "scripts"))
