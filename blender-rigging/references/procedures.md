# Tested procedures: blender-rigging

Every block ran on Blender 5.2.1 LTS with `blender -b --factory-startup` (and works in a live session). Tests live in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-rigging/`; `run_all.sh` runs the whole suite (about 2.5 minutes, 16 runs, all passing on 2026-09-24). Outputs and renders land in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-rigging/out/`.

Import pattern:
```python
import sys
sys.path.append("<skills>/blender-rigging/scripts"); sys.path.append("<skills>/blender-expert/scripts")
import bx_rig as R
```
...
```python
L, Rk = R.split_shape_lr(face, "mouth_smile")            # mouth_smile.L/.R, groups sum to 1 across the centre
...
```
Body = primitives joined by `bx_sculpt.union_remesh` (one manifold shell, about 19k vertices), eyes = two sphere islands, shirt = offset copy of the torso region (an overlapping garment). Real production mesh for tests: Snow in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/fixtures/retopo_examples.blend` (CC-BY Blender Studio), objects `snow_main`, `GEO-snow_shirt`, `GEO-snow_pants`, `GEO-rain_shoes.002`, `GEO-snow_eye_anim`, hair and brows.
...
(From the rigify_studio digest probes, `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/rigify_probe/`, and this skill's tests.)
