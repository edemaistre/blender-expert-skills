...
Tests live in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-hair/` (`run_all.sh` runs all of them). The helpers come from
`skills/blender-hair/scripts/bx_hair.py`. Each procedure shows the helper call, then the raw
bpy it wraps, so you can work without the module.

```python
import sys
sys.path.append("<project>/skills/blender-hair/scripts")
sys.path.append("<project>/skills/blender-expert/scripts")   # review() reuses bx_review
import bx_hair as H
```

Units are metres. Density counts curves per square metre of the surface mesh in its local space: object scale is ignored (verified), so apply scale first.
...
Verified on 5.2.1 by `test_01_groom_headless.py`. It passes 25 checks: Poisson spacing is respected (min nearest neighbour 0.01003 m at spacing 0.01), all roots fall inside the mask, `surface_uv_coordinate` is written, roots sit on the surface with max distance 0.0, and flow_blend 0.5 gives guides at 45° to the normal.
...
**Animal regional stack (Schmidbauer).** Verified on 5.2.1 by `test_09_animal_regions.py`. Each masked effect acted inside its painted region and barely outside it. Tip motion inside vs outside: Trim 1.0 mm vs 0.06 mm, Clump 7.3 mm vs 0.4 mm, Frizz 1.2 mm vs 0.09 mm. Tip Spread is a distance in metres: 0.2 threw 3 cm tips 19 cm. Use one curves object per landmark, and give each effect its own vertex group on the shared body mesh:
```python
...
4. The profile: an arc of angle Bend centred on +X and shifted so its apex sits on the curve, or a flat Spiral for tubes (resolution 6, 1 rotation, height 0, start radius = end radius; Matsumoto). A Switch picks one, and a Capture of Spline Parameter on the profile gives U.
