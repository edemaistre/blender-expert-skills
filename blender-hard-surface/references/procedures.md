...
Tests live in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-hard-surface/`:
...
Renders and JSON: `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/hard_surface/skill_out/` (`parts/`, `failures/`, `sheet_*.png`).

## P0. Import and conventions
```python
import sys
sys.path.append("<skills>/blender-hard-surface/scripts")   # also puts blender-expert/scripts on the path
import bx_hardsurface as HS
```
World-space metres. Objects are expected at scale 1 (`HS.apply_scale`). Cutters go to a `Cutters` collection, helper objects (normal sources) to `HS_Helpers`. The module never touches objects it was not given.
...
```python
bad = HS.nonplanar_faces(ob)          # [(face index, deviation in metres)], tolerance 1e-4 x size
...
- SubD high to low (rileyb3d): same object at level 1: 25,728 to 6,432 tris; dissolve non-silhouette loops and bake in blender-uv-baking.
- Multires bake, Bevel-shader-node bake and painted 32-bit height (PzThree, Augusto): see blender-uv-baking; not run here.
...
```
Modes: `matcap` (clay_studio), `normals` (check_normal+y), `wire`, `silhouette` from bx_review, plus `shiny` (metal_carpaint), `reflect` (check_reflection_horizontal), `hsgrey` (hard_surface_grey), `fullmetal`. `review` bakes the evaluated mesh into temporary objects, so the wire row shows the real boolean topology (Exact's split edges across n-gons are normal).
...
- `bpy.ops.mesh.extrude_manifold`, `select_similar`, `select_edge_ring_multi`: op ids verified by the distiller, behaviour not run.
- Sculpted damage on multires (PzThree): GUI with bx_gui, see blender-sculpting.
