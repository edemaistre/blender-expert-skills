...
Distilled from nine videos (notes in `notes/hard-surface/`, digest `_digest_hard_surface.md`). Timestamps are `[hh:mm:ss]` in the named video. Items marked [verified] were re-run in Blender 5.2.1 for this skill (tests in `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-hard-surface/`); [added] marks conclusions that are this skill's, not an expert's.
...
- Mirror the cutter, not the target, with its origin at world centre [00:14:53], [00:15:27]. Union instead of Join so two pieces share a bevel [00:18:57]. Smooth-shade the cutter [00:25:26]. Make a slice single user before applying [00:31:51].
