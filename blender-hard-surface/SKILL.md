...
**REQUIRED BACKGROUND:** blender-expert (execution channel, review loop, 5.2 API changes). Toolkit: `scripts/bx_hardsurface.py` (`import sys; sys.path.append("<skills>/blender-hard-surface/scripts"); import bx_hardsurface as HS`), world-space metres, stock Blender only.
...
4. **Design pass.** Cluster details, leave calm areas, align with neighbouring features (Ryuu [00:28:37], Gambrell [00:34:15]); hide perfect intersections behind a collar (du Mont [00:39:36]); union parts that must share a bevel (Gambrell [00:18:57]). GATE: a review sheet shows intent, not noise.
...
- `ob.modifiers.move(i, j)` silently refuses moves that involve pinned modifiers; `modifier_move_to_index` returns CANCELLED. Unpin, move, re-pin bottom-up.
