...
- A theatre version exists (or a reason for skipping it) and was judged for acting before cameras.
- Runtime is budgeted: cinematic about 2x theatre, up to 3x; about 1.6 to 1.9 s per beat for a first cinematic cut [derived]. Too long means gags over-milked; too short means beats missing.
...
- Cost: Workbench greys, no mood lighting (a non-Workbench engine is an audit problem, lights a warning); mannequins unless the brief asks for Rigify (then `ensure_rigify` first).
...
- **Cheats invisible:** no hovering feet, clipped props or giant furniture in frame; the neighbouring shots unchanged (the cheat was keyed).
...
- Neutral grammar where nothing is meant: eye level, medium lens, centred, static.
...
- Greys, one pop colour for the hero; opaque passepartout on every shot camera (audit), nothing outside the frame counts.
...
Built by `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/tests/code/blender-previs-storyboard/test_01_s6_end_to_end.py`. 15 beats (two added and marked), a theatre pass (10 blocks, one 35 mm camera at 1.03 m, the mean of the two eye heights, 11.8 s) and a cinematic cut (11 shots, 24.5 s = 2.1x the theatre pass, 1.63 s per beat).
...
**What the review caught** (outputs of each run kept under `/Users/emmanuel/Developer/pro/2026-09-24 Blender Expert Skills/archive/tests/previs/skill_out_run1/`):
...
| 1 | Insert too tight: two coloured blocks, no hand | no | framed robot and snack together |
...
| 3 | All shots read; theatre robot tiny (0.55 m at 6.6 m: 15 % of frame height) | yes (size) | expected: the reason the cinematic cut drops to the robot's eye line |
