...
Toolkit: `scripts/bx_previs.py` (tested on 5.2.1): `import sys; sys.path += ["<this skill>/scripts", "<blender-expert>/scripts"]; import bx_previs as P`.

**Which skill.** This one: story previs and animatics judged by a director and an editor. **text-image-to-blender-blockout** instead when the playblast guides a render-to-real video model (Seedance): saturated colour identities, no lens jump on a cut, eased moves, a hand-off sheet; an approved animatic that must become a video guide is rebuilt there. Shared: metric Z-up, roots at the feet, one camera per shot bound to a marker, `to_track_quat('-Z','Y')` look-at. Strokes: blender-grease-pencil. Animation after layout: blender-animation. Real rigs: blender-rigging.
...
- **Hjalti: theatre first, then cinematic; shots cost time.** A locked-off version judges the acting; the same beats told in shots ran about 2x (up to 3x) because every cut must be read (13/22/25 s vs 47/40 s). The oversimplified version is always too short and hides logistics (a character cannot cross the room in 10 frames).
...
- **Previs is the cheapest version that still conveys the idea** (Hjalti, Charge). Animate only shots whose camera move is the storytelling; everything else is one still, push-ins faked on the still in the edit. Greys in Workbench, no mood lighting, mannequins before rigs. `P.audit` refuses a camera moving inside a still shot, keys between block starts and a non-Workbench engine.
...
| Treatment | shot count, runtime | theatre pass, then cinematic |
...
2. **Set and proxies.** `P.setup_scene` (format, Workbench, greys, Standard view transform); `P.prop` for real-scale set pieces; `P.mannequin` for characters (root at the feet with `eye_height`, four pivot controls as the layout subset). Greys, one pop colour for the hero (Renato: red). GATE: scale reads next to a door (2.1 m) and counter (1.0 m).
3. **Theatre pass.** One locked-off camera (`cam` shared, `fit="all"`, 35 mm, mean eye height) framing the action's extremes; one block per beat group. GATE: in the `P.audit` rows every subject's `head_in` and `feet_in` is true at every block; theatre edit timed.
...
5. **Blocking.** For each block: `sc.frame_set(f)` (holds the previous pose), set only what changes, `P.key_block(all_movables, f)`; cheats go through the same keys; then `P.make_stepped`. GATE: `P.stepped_report == {}`; the cheat check at neighbouring shots.
...
| 35 mm theatre frame; about 60 mm for an isolating key shot | 36 mm sensor | Hjalti |
| F-stop 5.6, clip 0.01 to 200 m on the camera rig | Hjalti's rig | Hjalti |
| 13 / 22 / 25 s theatre vs 47 / 40 s cinematic | same script | Hjalti slide 00:46:01 |
...
|---|---|---|
| Eye level, medium lens, centred, static | neutral; the start for every A/B | Dillon, Hjalti |
...
| Off-centre from behind | isolation | Dillon |
...
| Every shot animated, mood lighting, full rigs | slow, the edit fights the moves, nothing judged sooner | stills plus `push`, `live` only when the move is the meaning, Workbench greys, mannequins |
...
- `references/procedures.md`: tested code for every stage (with and without the module) and the trap tests; load before writing code.
- `references/expert-notes.md`: each expert's principles, disagreements and deciding conditions, with timestamps; load when choosing a treatment, lens or cut.
- `references/critique.md`: the review rubric and the S6 findings; load at every Look gate.
- `references/sources.md`: the seven source videos, what each is best for, best timestamps.
